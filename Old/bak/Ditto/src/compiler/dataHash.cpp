...
private:
	...
	int findPos(char* string){
		...
		//Find the positon.
		while(dataPool.pool[(hashVal + j) % dataPool.size].data != NULL)
			j *= j;
	...
	int locateObj(char* name, long *location, dobject **object){
		...
		while(dataPool.pool[(hashVal + j) % dataPool.size].data != NULL){
			...
			if(tools.fullstrcmp(dataPool.pool[(hashVal + j) % dataPool.size].name,name)){
				...
				*object = &dataPool.pool[(hashVal + j) % dataPool.size];

				//Set the location object to the actual location.
				*location = (hashVal + j) % dataPool.size;
...
public:
	...
	void put(short type, char* name, char* data){
		int size, nameSize, pos;
		int i = 0;
	...
	int get(char* name, long *location, short** type, int** size, char** value){
		dobject *temp;
		if(locateObj(name,location,&temp)){
			*value = temp->data;
			*type = &temp->type;
			*size = &temp->size;
			return 1;

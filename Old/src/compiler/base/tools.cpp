...
public:
	...
	//Maximum number for the number of given bytes.
	long maxByteNumber(int numberOfBytes){
		int i;
		long max = 1;
	...
	//Break a double into two distinct integers...
	int breakDouble(long double input, long* whole, long* decimal, short* places){
		unsigned int i;
		long first, second;
		long double secondD;
	
		//Store the whole number part of the double as an integer.
		first = (long)input;
		secondD = input - first;	
	
		
		//Convert the decimal portion to a whole number.
		for(i = 0; i < 10; i++)
			secondD *= 10;
		...
		//Correct for the number of places.
		i = 10;
		while((second % 10) == 0 && i > 0){
			second /= 10;
			i--;
		}
	...
	//Recombine a double...
	long double rebuildDouble(long whole, long decimal, short places){
		short i,negative;
		long double result;
		...
		//Check to see if this is a negative double...
		if(whole < 0){
			negative = 1;
			whole *= -1;
		}

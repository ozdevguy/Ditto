...
public:
	...
	*/
	...
	//This function allocates space for a group of integers, then returns the memory location.
	int allocateInt(long allocationSize, int dataSize, int* data = NULL){
		int i, j, location;
	...
	int allocateString(int allocationSize, int dataSize, char* data = NULL){
		int i, j, location;
	...
	*/
	...
	char pullChar(int location,int offset = 0){
		short headerSize, i;
	...
	int pullInt(int location,int offset = 0){
		short headerSize, i;
		unsigned long dataSize, dataLoc;
		short type;
		int value = 0;
	...
	long double  pullDouble(int location,int offset = 0){
		int whole, fraction, places;
		...
		if(location < locationPointer){
			if((offset / 3) < pool.memory[location + 1]){
				places = pool.memory[location + offset + 2];
				whole = pool.memory[location + offset + 3];
				fraction = pool.memory[location + offset + 4];
				return tools.rebuildDouble(whole,fraction,places);
			}
			return -1;
		}
		return -1;
